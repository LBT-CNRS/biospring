...
# Run the BioSpring simulation in Docker
...
run: $(NC) $(MSP)
	...
	echo ""; \
	echo "$(YELLOW)Before continuing, please note that the BioSpring program is running now in idle mode within the container, waiting for a connection. By proceeding, we will reattach to the container and display BioSpring's logs in real-time. Afterward, you can use the following information to connect:$(RESET)"; \
	echo ""; \
	...
	trap 'docker ps -q --filter "id=$$CONTAINER_ID" | grep -q . && docker stop $$CONTAINER_ID || true; \
		echo "$(YELLOW)Program interrupted manually (SIGINT/SIGTERM).$(RESET)"; exit 130' INT TERM; \
	while true; do \
...
usage:
	...
	@echo "Usage:"
	@echo ""
	@echo "Classic (local): if biospring is installed locally and model.nc/param.msp"
	@echo "already exist, just run:"
	@echo "  make run / make run_now"
	@echo ""
	@echo "Docker (no local install needed):"
	@echo "Step 1: Server side"
	@echo "  make -f Makefile.docker prep"
	@echo "  make -f Makefile.docker expose_data"
	@echo ""
	@echo "Step 2: Client side"
	@echo "  make -f Makefile.docker get_data"
	@echo ""
	...
	@echo "  make -f Makefile.docker clean"
	@echo "------------------------------------------------------------------------------"
