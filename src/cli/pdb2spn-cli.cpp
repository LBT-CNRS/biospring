...
void biospring::pdb2spn::reduceToCoarseGrain(topology::Topology & top, const CommandLineArguments & args)
{
    ...
    // Springs the input file already carried (a coarse-grained PDB's CONECT
    // records) are re-drawn between grains by the reducer itself; this
    // assignment then transfers them by unique id, not by position. Reading
    // them back off `top` before this line would still see the pre-reduction
    // ones.
