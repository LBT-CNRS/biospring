...
class Topology
{
  ...
  public:
    ...
    // Assignment operator.
    //
    // Plain assignment: after it, this topology IS `other`. It used to keep
    // this topology's own springs whenever the two particle counts happened to
    // match, so that `top = reducer.target_topology()` would carry the springs
    // parsed from the input PDB's CONECT records across the reduction. That
    // could not work: _copy_particles replaces the particle list in place,
    // while a Spring holds a Particle& into it, and the reducer emits grains
    // in rule order -- so every kept spring silently moved to an unrelated
    // pair of particles, keeping a correct-looking equilibrium all the while.
    // A B-DNA duplex's 48 base-pair CONECT springs all ended up on wrong
    // atoms, at rest lengths of 2.8-3.1 A between atoms 5-13 A apart.
    //
    // Carrying springs across a reduction needs the atom-to-grain mapping,
    // which only the reducer has: it now re-draws them itself, see
    // reduce::Reducer::_carry_springs_over.
