...
Matrix& Matrix::operator*=(const Vector3f& v)
{
    // Checks if the number of lines is equal to 3 otherwise returns an error
    if (rows_ != 3) {
        throw std::invalid_argument("Matrix must have 3 rows");
    }
    
    Matrix temp(rows_, 1);
    temp(0, 0) = p[0][0] * v.getX() + p[0][1] * v.getY() + p[0][2] * v.getZ();
    temp(1, 0) = p[1][0] * v.getX() + p[1][1] * v.getY() + p[1][2] * v.getZ();
    temp(2, 0) = p[2][0] * v.getX() + p[2][1] * v.getY() + p[2][2] * v.getZ();
    return (*this = temp);
}
...
{
    if (rows_ != 3 || cols_ != 1) {
        throw std::invalid_argument("Matrix must have dimensions 3x1");
    }
    return Vector3f(p[0][0], p[1][0], p[2][0]);
}
...
Matrix operator*(const Matrix& m1, const Vector3f& v)
{
    Matrix temp(m1);
    return (temp *= v);
}
